...
容易混淆的一点是：**封装、继承、多态不是“抽象”本身**。通常的说法是：

```text
抽象：面向对象的建模思想
封装、继承、多态：面向对象的重要特征和实现机制
```

#### 什么是抽象

抽象就是从复杂的现实事物中提取当前问题真正需要的共同属性和行为，忽略暂时不关心的细节。

例如开发学生管理系统时，不需要记录一个人的所有现实信息，只抽取系统需要的部分：
...
这里的 `Student` 类不是某一个具体学生，而是对“学生这一类事物”的抽象；`Student s = new Student()` 才是一个具体对象。

#### 封装如何配合抽象

抽象决定“模型里保留哪些属性和行为”，封装决定“这些细节如何被保护”：
...
外部只需要通过 `setName` 修改姓名，不必知道成员变量如何保存。封装隐藏实现细节，让抽象出来的对象更容易使用和维护。

#### 继承如何利用抽象

继承把多个类的共同属性和行为抽取到父类，子类复用这些共同内容：
...
`Animal` 抽象出动物共有的行为，`Dog` 在此基础上增加自己的行为。继承不是简单复制代码，而是表达“Dog 是一种 Animal”的分类关系。

#### 多态如何配合抽象

多态允许使用父类或接口类型引用不同的子类对象，调用同一个方法时执行各自的实现：
...
调用者只依赖 `Animal` 这个抽象，不需要知道具体对象是哪一种动物。这样可以降低代码耦合，更容易替换实现。

#### 抽象类是另一个具体语法概念

“抽象”是思想；`abstract class` 是 Java 中的一种语法工具，不能直接创建对象：
...
抽象类可以定义抽象方法，要求子类提供具体实现。抽象类只是实现抽象思想的一种方式，接口也可以承担类似的抽象职责。
...
抽象思想很重要，因为程序是在用代码模拟现实或业务。抽象能力决定了能否把复杂问题拆成清晰的类、对象和方法。

在学生管理系统中，程序不需要记录学生的所有现实信息，只抽取业务真正需要的 `id`、`name`、`age`、`address`；在后端项目中，也会把用户、订单、商品、权限等业务概念抽象成类和接口。

抽象带来的好处：

- 降低复杂度：只关注当前问题需要的部分。
- 提高复用性：共同属性和行为可以放到父类或接口中。
- 降低耦合：调用者依赖抽象，而不是依赖具体实现。
- 方便扩展：增加新类型时，尽量不修改原有调用代码。
- 便于协作：类和方法的职责清楚，别人更容易使用。
...
使用者只需要知道“这是一个保存学生的列表”，不需要知道 `ArrayList` 内部如何扩容和移动数组元素。这就是类库通过抽象隐藏实现细节。

但抽象也不能过度。刚开始需求还不明确时，不要为了“看起来高级”提前设计很多父类和接口；先识别真实的共同点，再提取抽象。好的抽象应该让代码更清楚，而不是增加理解成本。
...
抽象帮助我们把复杂问题拆成清晰的模型、类和职责；逻辑帮助我们正确组织条件、循环、数据流和执行顺序。后续的语法、框架和工具，都是用来表达和实现这两种能力的。

补充：代理也是 Java 后端中很重要的技术，例如 Spring AOP、事务、权限检查和日志功能都可能使用代理。但代理更像是建立在抽象、接口、封装和多态之上的一种设计模式或实现手段，不是最底层的编程能力。可以把它理解为：调用者访问代理对象，代理对象在调用真实对象前后增加额外逻辑。

这里还可以从更广的角度理解“代理”：现实世界中很多事情不是由使用者直接完成，而是通过代理人、中介或委托关系完成。编程中对应的概念包括代理（Proxy）、委托（Delegation）、间接层和接口：调用者依赖一个稳定的中间约定，具体实现由后面的对象完成。

因此，如果讨论软件设计中非常重要的思想，“抽象、委托”也可以作为一组有代表性的关键词；如果讨论最基础的编程能力，我仍使用“抽象、逻辑”，因为代理和委托需要先建立在抽象、接口和多态之上。

### 32. 多级继承：C 能否使用 A 中的方法
...
继承具有传递性。创建 C 对象后，C 通常可以直接调用从 B、A 继承来的可访问方法：
...
C c = new C();
c.methodA(); // 可以调用 A 中继承来的 public 方法
```
...
```text
public      子类和其他类都可以访问
protected   子类可以访问，包外有条件访问
默认权限    同一个包内可以访问
private     只有 A 自己可以直接访问，C 不能直接访问
```

`private` 成员虽然属于 A 对象的一部分，但不会被 C 直接访问；A 可以提供 `public` 或 `protected` 方法，让子类间接使用它。

构造方法不会被继承。创建 `C` 对象时，会先调用父类构造方法，再执行子类构造方法；如果 A 没有无参构造方法，B 或 C 就需要通过 `super(参数)` 显式调用合适的父类构造方法。
...
这和 `C c = new C(); c.methodA();` 是两件事：前者创建 A 对象，后者创建 C 对象并使用继承来的 A 方法。

### 33. 构造方法是什么

构造方法是对象创建时自动执行的特殊方法，主要负责给新对象进行初始化。调用 `new` 时，Java 会先创建对象，再自动执行对应的构造方法。

例如 `Student` 类中的无参构造方法：
...
```java
Student s = new Student("001", "张三", "18", "北京");
```
...
- 方法名必须和类名完全相同。
- 没有返回值类型，连 `void` 也不能写。
- 不能手动通过普通方法方式调用，通常由 `new` 自动调用。
- 可以重载，定义多个参数不同的构造方法。
- 如果类中完全没有写构造方法，Java 会自动提供一个无参构造方法。
- 如果自己写了有参构造方法，Java 就不会再自动提供无参构造方法；需要 `new Student()` 时必须自己补写无参构造方法。
- 构造方法不会被子类继承，但创建子类对象时会先调用父类构造方法。

构造方法和普通方法的区别：

```text
构造方法：创建对象时自动执行，负责初始化对象，不能写返回值类型
普通方法：对象创建后按需调用，可以有返回值，也可以没有返回值
```

`this` 在构造方法中表示正在创建的当前对象：
